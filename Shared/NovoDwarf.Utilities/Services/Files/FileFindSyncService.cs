...
public partial class FileService
{
	...
	private void Traverse(string path, string searchPattern, ConcurrentBag<string> result)
	{
		try
		{
			foreach (var file in Directory.EnumerateFiles(path, searchPattern)) result.Add(file);

			foreach (var dir in Directory.EnumerateDirectories(path)) Traverse(dir, searchPattern, result);
		}
