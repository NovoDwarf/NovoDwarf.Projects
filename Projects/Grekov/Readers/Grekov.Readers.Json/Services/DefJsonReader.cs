...
public sealed class DefJsonReader : IDefFormatReader
{
	...
	private void ReadDefinitions(DefReadContext context, JsonNode root, List<Def> result)
	{
		switch (root)
		{
	...
	private Def ReadDefinition(DefReadContext context, JsonObject obj, Type type, string fallbackId)
	{
	...
	private static bool TryGetContainerArray(JsonObject obj, out JsonArray array)
	{
		...
		{
			if (GetProperty(obj, property) is not JsonArray found) 
				continue;
