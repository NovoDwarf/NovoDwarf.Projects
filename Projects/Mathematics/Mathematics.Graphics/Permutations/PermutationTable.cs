...
public class PermutationTable
{
	...
	public PermutationTable(int size = 256, int seed = 0) => Permutate(size, seed);
	...
	public PermutationTable WithSeed(int newSeed) => new(Size, newSeed);

	private void Permutate(int size, int seed)
	{
		Size = size;
		DoubleSize = size * 2;
