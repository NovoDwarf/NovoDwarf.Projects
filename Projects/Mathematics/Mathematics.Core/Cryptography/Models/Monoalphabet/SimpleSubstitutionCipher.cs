using Mathematics.Core.Cryptography.Interfaces;

namespace Mathematics.Core.Cryptography.Models.Monoalphabet;
...
public class SimpleSubstitutionCipher : ICipher<byte[]>
{
	...
	public SimpleSubstitutionCipher(Dictionary<byte, byte> substitutionMap)
	{
		...
		foreach (var pair in substitutionMap) decryptionMap[pair.Value] = pair.Key;
	}
	...
	public byte[] Encrypt(byte[] data)
	{
		...
		for (var i = 0; i < data.Length; i++) encrypted[i] = encryptionMap.GetValueOrDefault(data[i], data[i]);
