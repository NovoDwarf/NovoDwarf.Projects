...
public sealed class GraphNodeViewModel : ViewModelBase
{
	...
	public double X
	{
		...
		set
		{
			...
			Model.Position = new Aegis.Numerics.Primitives.Float2((float)value, Model.Position.Y);
			OnPropertyChanged();
	...
	public double Y
	{
		...
		set
		{
			...
			Model.Position = new Aegis.Numerics.Primitives.Float2(Model.Position.X, (float)value);
			OnPropertyChanged();
			OnPropertyChanged(nameof(Position));
		}
	}
