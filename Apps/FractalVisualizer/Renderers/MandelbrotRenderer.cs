...
public class MandelbrotRenderer : IDisposable
{
    ...
    public void Render(FractalControls controls)
    {
        ...
        _glUtils.SetUniform("resolution", new Vector2(_gl.GetInteger(GetPName.MaxFramebufferWidth), _gl.GetInteger(GetPName.MaxFramebufferHeight)));
    ...
    private string GetVertexShaderSource() => File.ReadAllText("Shaders/Mandelbrot.vert");

    private string GetFragmentShaderSource() => File.ReadAllText("Shaders/Mandelbrot.frag");
