...
public class EditorPanel
{
	...
	private int _selectedAutocompleteIndex = 0;
	...
	public void Render()
	{
		...
		if (textChanged)
		{
			...
			var io = ImGui.GetIO();
			if (io.WantTextInput && _shaderCode.Length > 0)
			{
				_cursorPosition = _shaderCode.Length; // Approximation
			}
			...
			if (_shaderCode.Length > 0)
			{
				...
				{
					_autocompletePrefix = _autocompleteProvider.GetContextualPrefix(_shaderCode, _cursorPosition) ?? string.Empty;
					_currentSuggestions = _autocompleteProvider.GetSuggestions(_autocompletePrefix, _cursorPosition, _shaderCode);
		...
		if (_showAutocomplete && _currentSuggestions.Count > 0)
		{
	...
	public void SetShaderCode(string code) => _shaderCode = code;
}
