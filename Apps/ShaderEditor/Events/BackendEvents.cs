        ...
        try
        {
            ...
            if (hr.IsFailure)
            {
                if (vertexErrors.Handle is not null)
                    throw new Exception($"Vertex shader compilation failed: {SilkMarshal.PtrToString((nint)vertexErrors.GetBufferPointer())}");
            ...
            if (hr.IsFailure)
            {
                if (pixelErrors.Handle is not null)
                    throw new Exception($"Pixel shader compilation failed: {SilkMarshal.PtrToString((nint)pixelErrors.GetBufferPointer())}");
            ...
            SilkMarshal.ThrowHResult(device.CreateVertexShader(vertexCode.GetBufferPointer(), vertexCode.GetBufferSize(), 
