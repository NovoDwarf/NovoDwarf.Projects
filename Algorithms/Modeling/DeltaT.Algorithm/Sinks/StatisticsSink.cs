...
namespace DeltaT.Algorithm.Sinks;
...
public class StatisticsSink : IMetricSink
{
    ...
    private string DetermineNodeType(Dictionary<string, IMetricEvent> nodeMetrics)
    {
        ...
        if (nodeMetrics.ContainsKey("Queue_Size_History") || nodeMetrics.ContainsKey("Queue_WaitTime") || nodeMetrics.ContainsKey("Queue_Dropped"))
            return "Queue";
    ...
    {
        if (!nodeMetrics.TryGetValue("Service_Completed", out var serviceCompleted) || serviceCompleted is not CounterEvent completedCounter) 
            return;
    ...
    {
        if (!nodeMetrics.TryGetValue("Service_Duration", out var duration) || duration is not NumericListEvent { Count: > 0 } durationList) 
            return;
    ...
    private void AppendUtilizationMetrics(StringBuilder sb, Dictionary<string, IMetricEvent> nodeMetrics, double totalTime)
    {
    ...
    {
        if (!nodeMetrics.TryGetValue("Queue_Size_History", out var queueSizeHistory) || queueSizeHistory is not NumericListEvent { Count: > 0 } sizeList) 
            return;
    ...
    {
        if (!nodeMetrics.TryGetValue("Queue_WaitTime", out var waitTime) || waitTime is not NumericListEvent { Count: > 0 } waitList) 
            return;
    ...
    {
        if (!nodeMetrics.TryGetValue("Source_GenerationTime", out var genTime) || genTime is not NumericListEvent { Count: > 0 } genTimeList) return;
    ...
    {
        if (!nodeMetrics.TryGetValue("Source_BlockTime", out var blockTime) || blockTime is not NumericListEvent { Count: > 0 } blockList) return;
